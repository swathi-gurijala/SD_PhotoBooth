...
const filters = [
  "90s", "2000s", "Noir", "Fisheye", "Rainbow", "Glitch", "Crosshatch"
];
...
const PhotoStudio = () => {
  ...
  const [background, setBackground] = useState("soft.png");
  ...
  const getFilterClass = (filter) => {
    switch (filter.toLowerCase()) {
      case "90s": return "_90s";
      case "2000s": return "_2000s";
      default: return filter.toLowerCase();
    }
  };
  ...
  const handleDownload = async () => {
    ...
    link.href = dataURL;
    link.download = `SD_strip_${new Date().toISOString().slice(0, 10)}.jpg`;
    link.click();
  };

  return (
    <motion.div
      className={`photoStudio ${theme}`}
      style={{
    ...
    >
      ...
      {!showResult && (
        <div className="studio-container">
          ...
          <div className="filter-bar no-scroll">
            {filters.map((filter) => (
              <button
                key={filter}
        ...
        <div className="studio-result slide-in-top">
          ...
          >
            ...
            <p className="photostrip-caption">
              SD PhotoBooth • {new Date().toLocaleDateString("en-IN", {
          ...
          <div className="result-controls">
            <button onClick={handleReshoot} className="reshoot">Retake the pics</button>
            <button onClick={handleDownload} className="download">Download Strip</button>
          </div>
        </div>
      )}
    </motion.div>
